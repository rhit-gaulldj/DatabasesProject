...
BEGIN
	IF (@CourseId is null OR @NumResults is null OR @AllowDupAthletes is null
		...
		RETURN 1
	END
	-- If the course ID is < 0, we'll do for all courses
	-- But if course ID is >= 0, need to check that it exists
	IF (@CourseID >= 0 and (SELECT COUNT(*) FROM Course WHERE course_id = @CourseID) <= 0) BEGIN
		RAISERROR('Course does not exist', 14, 1)
		RETURN 2
	END

	-- Race level must exist
	IF ((SELECT COUNT(*) FROM RaceLevel WHERE race_level_id = @RaceLevelId) < 1) BEGIN
	...
	IF (@AllowDupAthletes = 1)
		SELECT TOP(@NumResults) v1.Athlete, v1.[FormattedTime], v1.[Meet], v1.[Year], v1.[Grade]
			...
			WHERE v1.course_id = @CourseID AND v1.race_level_id = @RaceLevelId AND
	...
	ELSE
		SELECT TOP(@NumResults) v1.Athlete, v1.[FormattedTime], v1.[Meet], v1.[Year], v1.[Grade]
			...
			JOIN (SELECT athlete_id, MIN([time]) as min_time
					FROM all_times_view
					WHERE course_id = @CourseID AND race_level_id = @RaceLevelId AND
